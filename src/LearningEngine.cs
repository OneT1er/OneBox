...
namespace PowerAudioManager
{
    ...
    /// 3) 若模型已训练且自动套用开启：每秒推理一次，预测结果连续 5s 不变才视为有效；与当前不同则自动切换，
    ///    切换后强制冷却 30s 防来回跳；手动切换后 10 分钟内不自动套用。
    /// 4) 样本累积达 200 条自动训练（每 +50 重训），也可在设置里手动训练。
    ...
    public static class LearningEngine
    {
        ...
        static void TryInfer(FeatureCollector.Snapshot s)
        {
            ...
            try
            {
                ...
                if (!DecisionTreeLearner.HasPowerModel && !DecisionTreeLearner.HasAudioModel) return; // 模型未就绪
        ...
        // 样本达阈值自动训练；每 +50 重训。手动训练由设置面板直接调 TrainAsync。
        static void MaybeAutoTrain()
        {
            if (_training) return;
            int n = SampleStore.Count;
            if (n < DecisionTreeLearner.AutoTrainThreshold) return;
            if (n != DecisionTreeLearner.AutoTrainThreshold && (n - _lastAutoTrainCount) < 50) return;
            _training = true;
            _lastAutoTrainCount = n;
