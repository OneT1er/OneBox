...
- 模块拆分（v1.2.0）：MainWindow + AppResources(字体/资源/共享深色样式) + LauncherBar + WindowScaling + TrayController + SettingsDialog + ScreenshotService/ScreenshotToast + Dialogs。配置走注册表 `HKCU\Software\PowerAudioManager\App`（AppPrefs）。
...
- **性能趋势图表**：`PerfHistory` 环形存 (值,时间戳)，仅存真实读数（`MetricValue.Cached` 兜底旧值跳过）；`PerfChart` 按时间戳映射 x，相邻点时间差超阈值（3 间隔/≥5s）断线--传感器失配/跨重启缺口显示为断口而非填旧值。时长档 5分/15分/30分/1时/2时/6时/12时/全天，默认 15 分。旧 JSON 无时间戳时按文件修改时间回填。 图表按需采集：PerfHistory/ForegroundHistory 仅性能趋势图打开时加载+记录（引用计数），关闭即 Save+释放内存；打开时窗口内无数据会自动锚到最后一条历史，以前记录的数据立即可见。
...
更完整的分条记忆在 `.Codex/memory/`（每文件一事实，含 frontmatter）：overview / build-constraints / user-workflow / architecture / pitfalls / hdr-screenshot / migration-gotchas。
