...
从悬浮窗「音频输出」中的 OneMic 图标或托盘菜单打开；悬浮窗还提供共享、降噪、EQ、BGM、炸麦、监听和设置图标快捷操作。
...
- 电平与频谱、现场延迟测试、统一紫影深色界面、托盘后台运行和可选登录后共享。OneMic 的启动与快捷键设置位于 OneBox 原有设置侧栏，「关于」固定在侧栏底部。
...
- 后台持续采集并每 60 秒原子落盘到 `%LocalAppData%\OneT1er\OneBox\`，该目录独立于安装器管理范围；保留上一代有效备份，异常断电后可自动恢复此前记录。
...
| 性能趋势历史 | `%LocalAppData%\OneT1er\OneBox\OneBox.perfhistory.json`，独立于安装目录，原子写入并保留有效备份 |
| 截图与图库 | 用户在设置中选择的目录 |
