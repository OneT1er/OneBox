...
### 修复与改进
- 麦克风工作室更名 OneMic；预设切换保持人声面板展开，设置并入 OneBox 侧栏并移除独立语言与空闲升级选项。
- 悬浮窗 OneMic 快捷操作统一使用矢量图标，提供打开、共享、降噪、EQ、BGM、炸麦、监听和设置；「关于」固定在设置侧栏底部。
- 修复共用 USB 容器时监听被直接禁用的问题；现在尝试打开耳机并显示实际驱动错误。
- 修复同一播放器的多个进程只按路径选中静音 PID 的问题；优先捕获有音频电平的进程，显示捕获 PID 与 BGM 电平，并提示通话软件正在使用的麦克风设备。
- Studio 设置并入 OneBox 设置侧栏，悬浮窗增加共享、BGM、监听快捷控制；提供 VB-Audio 官网原包下载入口。
- 增加 GTCRN 流式 CPU 降噪与 Off / Eco / Balanced / Quality 模式，Balanced 作为首次使用默认档；增加现场延迟测试与处理超时提醒。

### 新增
- OneMic：WASAPI Shared 实时麦克风采集，RNNoise / GTCRN / DeepFilterNet3 可扩展降噪，安静/标准/嘈杂/直播预设，强度与人声音量调节。
- 10 段人声 EQ（7 个音色预设、可拖拽曲线、自定义持久化），指定应用的立体声 BGM 混音，最终限幅后输出至虚拟麦克风。
- 五个监听位置、受限幅保护的炸麦音效，五项单键/组合键全局快捷键。
- 麦克风/输出/监听设备选择与断线恢复、电平表/频谱、OneBox 紫影深色界面、统一设置与独立教程。
- 复用 OneBox 托盘和开机启动；可选启动后共享。VB-CABLE 由应用直接下载官方原包，不捆绑驱动。
- DeepFilterNet3 使用上游 MIT/Apache 发布组件，隔离在后台子进程；音频模块独立实现，不包含 MeowMic 代码。

## v1.8.3 (2026-09-20)

### 新增
- GitHub Actions 自动构建与发布：推送版本标签后完成测试、打包并发布安装包、便携包和自动更新文件；支持手动构建。
- 截图通知支持四角位置、缩略版、1–30 秒显示时间与即时预览；默认右下角。可排除通知被系统截图捕获，OneBox 再次截图前自动关闭旧通知。
- 设置新增「关于」，显示当前版本、离线更新记录和 GitHub 项目地址。

### 修复
- 修复截图通知的位置被 WPF 初次布局覆盖、所有位置都停在左上角的问题；等待布局完成后按真实窗口尺寸定位。
- 性能趋势前台应用按真实采样覆盖范围显示；未采集、休眠或重启期间显示「未记录」，不再用当前应用填补历史。性能监控开启时后台持续记录实际前台应用，不再根据窗口层级猜测。

### 优化
- 截图通知采用更清晰的预览与状态布局，支持悬停暂停关闭、独立关闭按钮，并修正失败通知标题。
- 性能指标采用紧凑卡片，设置窗口统一深色滚动条与侧栏状态，操作按钮固定在底部。
