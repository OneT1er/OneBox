...
### 新增
- 麦克风工作室：WASAPI Shared 实时麦克风采集，RNNoise / DeepFilterNet3 可扩展降噪，安静/标准/嘈杂/直播预设，强度与人声音量调节。
- 10 段人声 EQ（7 个音色预设、可拖拽曲线、自定义持久化），指定应用的立体声 BGM 混音，最终限幅后输出至虚拟麦克风。
- 五个监听位置、USB 共用设备检测、受限幅保护的炸麦音效，五项单键/组合键全局快捷键。
- 麦克风/输出/监听设备选择与断线恢复、电平表/频谱、中文/英文、OneBox 紫影深色与暖调浅色界面、独立设置与教程。
- 复用 OneBox 托盘、开机启动及 Velopack 更新；可选启动后共享、空闲时静默更新。VB-CABLE 仅提供官方安装指引，不捆绑驱动。
